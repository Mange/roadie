module Roadie
  # An abstract base class which can be subclassed to implement an entry point
  # for Roadie.
  #
  # @api private
  # @abstract Subclass and override {#transform} to implement a custom document class.
  #
  # A subclass instance must never be used from two threads at the same time.
  # Reusing them is discouraged.
  #
  # Stylesheets are added to the input HTML from three different sources:
  ...
  #
  # @attr [#call] before_transformation Callback to call just before {#transform}ation begins. Will be called with the parsed DOM tree and the {DocumentBase} implementation instance.
  # @attr [#call] after_transformation Callback to call just before {#transform}ation is completed. Will be called with the current DOM tree and the {DocumentBase} implementation instance.
  class DocumentBase
    ...
    protected
    # Transform the input HTML and return the processed HTML. Subclasses must
    # implement this method.
    #
    # @abstract Subclass and override to implement the transformation of the
    # input HTML.
    #
    # Make sure to run the {#before_transformation} callback with the parsed
    # HTML tree and the subclass instance when the transformation begins, and
    # after all work is complete invoke the {#after_transformation} callback in
    # the same way.
    #
    # Most of the work can be delegated to other classes. A list of them can be
    # seen below.
    #
